...
def test_isolated_environment_install(mocker):
    with build.env.DefaultIsolatedEnv() as env:
        mocker.patch('build.env._subprocess_run')

        env.install([])
        build.env._subprocess_run.assert_not_called()

        env.install(['some', 'requirements'])
        build.env._subprocess_run.assert_called()
        args = build.env._subprocess_run.call_args[0][0][:-1]
        assert args == [
...
def test_pip_needs_upgrade_mac_os_11(mocker, pip_version, arch):
    ...
    _subprocess = mocker.patch('build.env._subprocess_run')
